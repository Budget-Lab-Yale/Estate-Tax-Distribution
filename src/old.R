...
inheritance_train = scf %>% 
  ...
  mutate(
    income_pctile = cut(
      x      = income,
      breaks = wtd.quantile(income, weight, c(seq(0, 0.99, 0.01), seq(0.991, 1, 0.002))) + runif(105), 
      labels = c(seq(0.01, 0.99, 0.01), seq(0.991, 1, 0.002))
...
inheritance_qrf = quantregForest(
  x        = nonzero_inheritance_train[c('age', 'married', 'income_pctile')],
  y        = log(nonzero_inheritance_train$inheritance), 
  nthreads = parallel::detectCores(),
  mtry     = 3,
  nodesize = 10
)
...
inheritance_yhat = tax_units %>% 
  ...
  mutate(
    ...
    inheritance = predict(
      ...
      newdata = (.),
      what    = function(x) exp(sample(x, 1))
    )
  ) 
...
# mutate(p_inheritance = p_inheritance * (target_count / sum(p_inheritance * weight))) %>% 
# ungroup()
...
inheritance_yhat %>% 
  summarise(
    source    = 'pred',
    n         = sum(weight) / 1e6,
    share     = sum(p_inheritance * weight) / sum(weight), 
    count     = sum(p_inheritance * weight), 
    count_mil = sum((inheritance > 1e6) * p_inheritance * weight),
    amount    = sum(inheritance * p_inheritance * weight) / 1e9, 
    avg       = weighted.mean(inheritance, p_inheritance * weight), 
    median    = wtd.quantile(inheritance, p_inheritance * weight, 0.5)
  ) %>% 
  bind_rows(
    inheritance_train %>% 
      summarise(
